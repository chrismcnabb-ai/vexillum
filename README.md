...
**The problem first gets worse.** 100 vendor reports join against the inventory and become roughly 900 distinct (application × package × vulnerability) tuples, because one vulnerable package is consumed by many applications.

**Then it gets much better.** Those 900 tuples collapse to about 22 distinct vulnerabilities, because every Java service in an estate pulls the same Jackson, Spring, and Netty stack. The same CVE arrives many times over.
...
The result is that **expensive reasoning scales with the number of distinct vulnerabilities, not with report volume or application count.** In week one the profile cache is empty and everything is work. By week eight, better than four in five arriving CVEs already have a profile and replay for free. The system gets cheaper the longer it runs.
...
The remediation view groups findings by **fix action** rather than by finding, and the difference is the whole point. In the prototype, 571 open findings resolve to eight distinct actions, and the top three close 81% of them.

Nobody can act on 571 individual tickets. "Bump the Spring Boot parent and the gRPC shading parent" is a sprint commitment somebody can say yes to.
